# DECISION-1 — The "serves immediately" bridge test asserted more than the frozen header promises

- Component: `apps/macos/` (Swift 6 / SwiftPM, macOS 26)
- Slice: sort-by-column build cell, round 1
- Request kind: **contract** (two-key CHANGE-REQUEST; implementer signed, the cell reviewer's
  `[contract]` finding 3 is the second key)
- Ruling: **APPROVED** — amend the frozen test; delete the bridge's internal wait
- Frozen file amended: `Tests/LessSheetKitTests/SortByColumnTests.swift` (ONLY).
  `Sources/Contracts/DocumentSession.swift`'s "Never blocks" clause **stands unchanged**.
- Twin: `apps/gtk/.aidev/DECISION-2.md` — same defect, same ruling, adjudicated together.
