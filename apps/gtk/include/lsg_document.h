 ...
 * the SINGLE place that wraps the core's two-lane `ls_*` access for the
 * viewer.
 *
 ...
 * as owned, display-ready cells; per-cell truncation + per-row oversized
 * markers
 * + the original (source) row number for the gutter; and close. A NETWORK URL
 * open produces the same `LsgDocument` and lives in <lsg_net_open.h> (adopted
 * on DONE). Find / jump / filter / streaming copy / Settings / dialect
 * override are LATER slices and are NOT part of this frozen surface yet (it
 * grows per slice).
 *
 ...
 *     pass-through-UTF-8 rule), so nothing a caller holds is tied to the
 * core's window-eviction lifetime.
 ...
 * case). The mapping is pinned by a frozen test against the real `LS_*`
 * values.
 */
typedef enum
{
 ...
 * or for an empty document (bytes_total == 0); 0.0 while bytes_total is
 * unknown (LS_BYTES_TOTAL_UNKNOWN); otherwise bytes_scanned / bytes_total. */
...
/* The gutter sentinel for a row whose source number is unavailable (outside
 * the materialized window / view range) — mirrors the ABI's LS_NO_ROW. */
 ...
 * [first_col, first_col + col_count), copying every cell out
 * (U+FFFD-sanitized). `row_count` is clamped to LS_WINDOW_MAX_ROWS and the
 * column range is clamped to the document's column count, so a wide document's
 * per-window fetch is O(visible columns), never O(column_count). NEVER scans;
 * synchronous-fast; safe on the UI thread for any row size (an over-large
 * source row is served as a bounded prefix and flagged oversized — see
 * `lsg_window_row_oversized`).
 *
 ...
 */
LsgWindow *lsg_document_set_window (LsgDocument *doc, guint64 first_row,
                                    guint32 row_count, guint32 first_col,
                                    guint32 col_count);
 ...
 * [0, col_count) — as a BORROWED, valid, U+FFFD-sanitized, NUL-terminated
 * UTF-8 string owned by the window (valid until `lsg_window_free`). Returns ""
 * for any out-of-range (row, col); never NULL. */
const char *lsg_window_cell (const LsgWindow *window, guint32 row,
                             guint32 col);

/* Whether the cell at WINDOW-relative (row, col) was cut by the core's
 * per-cell display cap (`ls_cell_truncated`) — a grid draws a per-cell "…"
 * indicator. The cut is display-only. false for out-of-range. */
gboolean lsg_window_cell_truncated (const LsgWindow *window, guint32 row,
                                    guint32 col);
 ...
 * U+FFFD — the display-boundary rule the ABI mandates for the UTF-8
 * pass-through path. `bytes` may be NULL only when `len` is 0 (yields "").
 * This is the single point the session marshals borrowed core bytes into
 * caller-safe strings.
 */
