 ...
 * MATH (slice 1). The custom O(viewport) grid is a `GtkDrawingArea` driven by a
 * hand-managed `GtkAdjustment` + `GtkScrollbar` (uniform row height => O(1)
 ...
 *
 * Everything here is ARITHMETIC over plain numbers (pixels as `gdouble`, rows as
 * `guint64`, widths supplied by the frontend's own monospaced measurement) — no
 * widgets, no display server, no text layout. It answers three grid questions:
 ...
 */
LsgRowSpan lsg_grid_row_window (gdouble scroll_y, gdouble viewport_h, gdouble row_h,
 ...
 * when that is <= LSG_GRID_MAX_ADJUSTMENT_UPPER, else the saturated cap. Always
 * >= 0; 0 for an empty document. */
gdouble lsg_grid_content_height (guint64 row_estimate, gdouble row_h);

/*
 * The top data row for a scrollbar `offset` in [0, upper], where `upper` is the
 * `lsg_grid_content_height` for (row_estimate, row_h). DIRECT mapping
 ...
 * for the same `upper` / `row_estimate` / `row_h`. DIRECT (`row * row_h`) while
 * unsaturated; PROPORTIONAL (`row / row_estimate * upper`) once saturated.
 * Clamped to [0, upper].
 */
...
/*
 * Whether a row-count ESTIMATE change should be applied to the adjustment upper
 * NOW, or DEFERRED. Returns FALSE when `new_estimate == old_estimate` (nothing
 * to do) or when `is_overscrolling` (applying mid kinetic/elastic overscroll
 * would perturb the live scroll — defer until it settles, mirroring the macOS
 * deferred-estimate-reload rule); TRUE otherwise.
 */
gboolean lsg_grid_should_apply_estimate (guint64 old_estimate, guint64 new_estimate,
 ...
 * sum at `first` so in-window columns draw at the same x as a naive full-prefix
 * draw. A viewport spanning every column yields the whole range at first_x 0
 * (identical to a non-windowed draw). Empty `widths` (n == 0) or a viewport
 * intersecting no column yields {0, 0, 0}. The work is O(the answer's
 ...
 * other column byte-identical to `current`. NEVER lowers a width (monotone) and
 * NEVER lets one column affect another (independence), so a horizontal scroll
 * that re-measures a column over the same vertical row window can never churn
 * its established width. Pairs whose column index is >= n are ignored. `out`
 * may not alias `current`.
 */
void lsg_grid_grow_widths (const gdouble *current, guint n,
 ...
 * the MAXIMUM UTF-8 CHARACTER count (`g_utf8_strlen`) over the `n_cells` sampled
 * cell strings and `header` (NULL / "" ignored), plus `padding`. Pure
 ...
 */
gdouble lsg_grid_column_width_estimate (const char *const *cells, guint n_cells,
                                        const char *header,
