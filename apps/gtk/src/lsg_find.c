 ...
 *      (Sources/LessSheetKit/FindLogic.swift): a plain-value state machine that
 *      NEVER touches the core. It composes a request from the draft, folds
 ...
 *
 *   2. The SEARCH BRIDGE — a C port of the `CoreDocumentSession` search methods
 ...
 * NumericGrammar): sign? ( digits ('.' digits?)? | '.' digits ) (e sign? digits)?
 * over ASCII, after trimming ASCII whitespace (0x09..0x0D, 0x20). */
gboolean
...
lsg_find_initial (void)
{
  ...
  /* display is all-zero: inactive, no current, total 0, not final, no progress,
   * no notice. */
  return session;
}
                 ...
                 guint32 column_count)
{
  ...
  /* PREDICATE. A column outside the document rejects (blink + shake) before any
   * core call; a hidden column is a legal target. */
  ...
  if (lsg_search_op_is_ordering (draft.op) && !lsg_numeric_is_numeric (draft.value))
    {
      out.outcome = LSG_FIND_REJECTED;
      return out;
    }
                   ...
                   LsgSearchDir nav_direction)
{
  ...
  if (snapshot.phase == LSG_SEARCH_PHASE_SCANNING)
    {
      d.has_progress = TRUE;
      d.progress = MAX (old.has_progress ? old.progress : 0.0, snapshot.progress);
    }
  else
    {
  ...
  if (snapshot.nav == LSG_SEARCH_NAV_EXHAUSTED)
    {
      if (snapshot.total == 0 && snapshot.total_exact)
        {
          /* Zero matches anywhere (scan complete): "No matches", landing clear. */
        ...
        {
          /* Exhausted a direction with matches known (or still scanning): wrap,
           * keeping the current landing until the wrap lands. */
...
lsg_document_search_poll (const LsgDocument *doc, LsgSearchSnapshot *out)
{
  ...
  switch (s.state)
    {
    case LS_SEARCH_SCANNING:  phase = LSG_SEARCH_PHASE_SCANNING;  break;
    case LS_SEARCH_DONE:      phase = LSG_SEARCH_PHASE_DONE;      break;
    case LS_SEARCH_CANCELLED: phase = LSG_SEARCH_PHASE_CANCELLED; break;
    default:                  return FALSE;   /* LS_SEARCH_IDLE -> no snapshot */
    }

  LsgSearchNavState nav;
  switch (s.nav)
    {
    case LS_SEARCH_NAV_SEARCHING: nav = LSG_SEARCH_NAV_SEARCHING; break;
    case LS_SEARCH_NAV_FOUND:     nav = LSG_SEARCH_NAV_FOUND;     break;
    case LS_SEARCH_NAV_EXHAUSTED: nav = LSG_SEARCH_NAV_EXHAUSTED; break;
    default:                      nav = LSG_SEARCH_NAV_NONE;      break;
    }
                                 ...
                                 guint32 first_col, guint32 col_count)
{
  ...
  /* Window lane: serialize with set_window / cell reads, then copy the borrowed
   * mask out immediately (never held past the next set_window / close). */
