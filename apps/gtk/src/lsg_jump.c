 ...
 *      (ViewerLogic.swift) TOGETHER WITH the two reject decisions macOS keeps in
 *      its app layer (`ViewerModel.submitJump` upfront out-of-range reject +
 *      `foldJump` after-scan short-land reject), lifted here so the grid's jump
 *      behavior is gate-verifiable headlessly. A plain-value state machine that
 *      NEVER touches the core: parse + validate the entered 1-based row, decide
 *      run-vs-reject, fold the core jump-scan poll into a monotone progress
 ...
 *      reaches the core handle through the non-frozen `struct _LsgDocument` seam
 *      (Slice 2). Jump is poll/control-lane — lockless (the core is internally
                 ...
                 gboolean filtered, guint64 pre_jump_first_row)
{
   ...
   * number is an ORIGINAL row and `rowcount` reports the filtered m (a different
   * domain) — a filtered jump never rejects, it clamps to the last match. */
  ...
  if (status.state == LSG_JUMP_DONE)
    {
      ...
      /* UNFILTERED short land: the scan clamped past EOF (landed < target) — the
       * target was past the last data row. Reject + re-anchor rather than land on
       * the clamp (the after-scan out-of-range reject). Suppressed while filtered:
       * `r` is a FILTERED index and `flow.target` an ORIGINAL row (not
       * comparable) — a filtered jump lands (clamped to the last match). */
...
lsg_document_jump_poll (const LsgDocument *doc)
{
  ...
  switch (s.state)
    {
    case LS_JUMP_SCANNING: out.state = LSG_JUMP_SCANNING; break;
    case LS_JUMP_DONE:     out.state = LSG_JUMP_DONE;     break;
    default:               out.state = LSG_JUMP_IDLE;     break;
    }
