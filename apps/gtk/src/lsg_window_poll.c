 ...
 * frontier advances) and whether to KEEP the loop alive. Touches no core state.
 */
...
lsg_window_poll_decide (LsgWindowPollInputs inputs)
{
   ...
   * desired range: its rows beyond the frontier become servable as the frontier
   * advances, so re-issuing grows the retained prefix. */
  d.reissue_window = inputs.window_is_short;

  /* Keep polling while there is still work to observe: the window is short (the
   * frontier has not caught up to the request) OR indexing is not yet complete
   * (the row-count estimate is still converging). Stop only once the window is
   * full AND the index is exact. */
