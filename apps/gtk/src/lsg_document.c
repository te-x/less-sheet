...
lsg_open_error_from_status (ls_status status)
{
  switch (status)
    ...
    case LS_ERROR_NOT_FOUND:
      return LSG_OPEN_NOT_FOUND;
    case LS_ERROR_PERMISSION_DENIED:
      return LSG_OPEN_PERMISSION_DENIED;
    case LS_ERROR_IO:
      return LSG_OPEN_IO;
...
LsgWindow *
lsg_document_set_window (LsgDocument *doc, guint64 first_row,
      ...
      for (guint32 ri = 0; ri < rc; ri++)
        {
          ...
          w->oversized[ri]
              = ls_row_oversized (doc->doc, abs_row) ? TRUE : FALSE;
          ...
          for (guint32 ci = 0; ci < cc; ci++)
            {
              ...
              w->truncated[idx]
                  = ls_cell_truncated (doc->doc, abs_row, abs_col) ? TRUE
                                                                   : FALSE;
            }
        }
    }
