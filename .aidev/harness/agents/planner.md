...
7. Run `bash .aidev/harness/freeze.sh "$PWD"` to snapshot the protected surface — cover the signed
   architecture docs,
...
needed, write `.aidev/DECISION-<n>.md`, and re-run `bash .aidev/harness/freeze.sh "$PWD"`.
