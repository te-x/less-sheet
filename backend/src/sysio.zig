//! Portable blocking file I/O and thread synchronization, replacing the
//! macOS-only `std.c` libc bindings so the core cross-compiles to
//! aarch64/x86_64-linux-musl while staying byte-identical on macOS.
//!
...
//! they compile and work on macOS + Linux and are the named Windows plug-in
//! point. This module never touches them.
...
/// Wrap a raw POSIX fd as a blocking `File`. The fds the core opens never set
/// O_NONBLOCK, so `nonblocking` is always false.
...
/// A process-unique token for temp-file names. Folds a monotonic per-process
/// counter with the caller thread's id so concurrent creators — even at the
/// same object address across processes — get distinct names; the
/// O_CREAT|O_EXCL open is the final backstop against any residual collision.
