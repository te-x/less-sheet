//! The asynchronous URL-open job behind the ls_open_url_* C ABI and the
//! openUrlStartFake test seam. See docs/architecture/ARCH-network-source.md.
//!
...
/// asked to cancel IS the cancellation: the worker is a cancellable task (see
/// `NetOpenJob.future`), so an interrupted syscall surfaces as an ordinary error
/// (`error.Canceled` -> a failed fetch / a failed spool write), and reporting
/// that as FAILED/IO would contradict the state `cancel` already published.
///
...
/// Any new terminal path must either call `failLocked` or repeat the cancel-wins
/// check — and if a third inline site is ever needed, lift the shared part into a
/// helper that can carry `http_status` instead of adding another copy.
...
fn publish(job: *NetOpenJob, built: ?net_source.BuiltSource, build_err: api.NetStatus) void {
    ...
    defer job.unlock();
    job.doc = @ptrCast(doc);
    // Report the REAL head bytes fetched and the known total (0 when the length
    // is unknown), never fetched = total = file_size: for a streaming doc that
...
/// Live-progress trampoline: net_source calls this on every actual network fetch
/// (never a cache hit) with the running (bytes fetched, total). Updates the job's
/// poll-visible progress/bytes fields so `ls_net_open_poll` reports REAL
/// incremental progress, not just a start/terminal snapshot. A no-op once the job
/// has already reached a terminal state (a late callback from a build that raced
/// a cancel must not resurrect a stale `.fetching`).
...
fn runFake(job: *NetOpenJob, fx: *const api.NetFixture) void {
    ...
    // O(head) open scan must fetch, and open's latency is bounded by user cancel
    // (`ls_open_url_cancel`). Its own scope, not the worker's: the permit is
    // thread-local, and this body runs SYNCHRONOUSLY on the caller's thread
    // (`startJob` calls it inline) while `realWorker` runs on an executor thread —
    ...
    // A redirect whose Location downgrades the transport https->http is refused
    // with a distinct code. The fixture models the downgrade condition; routing it
    // through the PURE decision (net_source.redirectDowngrades) pins the taxonomy
    // mapping the real std.http.Client redirect path uses too (same seam,
    ...
    if (fx.stall) {
        // A live, non-terminal open: cancel is the only way out.
        job.state = .fetching;
        return;
    }
    // Fake classification, mirroring decideProbe over the fixture's server shape:
...
/// the probe AND every subsequent ranged fetch — no fresh TCP/TLS handshake per
/// 256 KiB chunk.
fn realWorker(job: *NetOpenJob) void {
    ...
    const progress: net_source.Progress = .{ .ctx = job, .callback = onProgress };
    // Every case streams through the http_range Source (never a full download):
    // `probe.range` picks random vs sequential fill, and a gzip resource composes
    // the gzip Source over that spool — buildNet detects the magic on the fetched
    // head.
    var build_err: api.NetStatus = .io;
...
pub fn release(job: *NetOpenJob) void {
    // If a real fetch is still in flight, signal cancel and join it first. Never
    // closes job.doc.
    job.cancel_flag.store(true, .release);
    if (job.future) |*f| {
